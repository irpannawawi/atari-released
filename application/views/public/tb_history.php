	...
	function search(event){
		...
		if (event.keyCode === 13 && $('#searchBox').val() != '') {
			...
			var keyword = $('#searchBox').val();
				...
				loading.fadeIn(500);
					$.post("<?=site_url('History_produksi/history_search')?>", {'keyword':keyword}, function(data){
						table.html(data);
						table.show(100)
						loading.fadeOut(500)
					});
      ...
      <div class="modal-body">
        <div class="col filter">
			<table class="table table-borderless ">
				...
				<tr>
					<td>
						<select id="jenis_input"  class="form-control">
							<option value="" id="defaultPartNoValue">Jenis Input</option>
						<?php foreach ($list_jenis_input as $key) { ?>
							<option value="<?=$key->jenis_input?>"><?=$key->jenis_input?></option>
						<?php } ?>
						</select>
					</td>
				</tr>
...
<section class="content">
	...
	<div class="col-12" id="table-wraper">
		<div id="headeer-page" hidden>
			...
			<hr />
			<h4 align="center">Histori Produksi</h4>
		</div>
...
<script type="text/javascript">
	...
	//filter menu action
	var url = '<?=site_url('History_produksi/history_filtered')?>'
	var url_search = '<?=site_url('History_produksi/history_search')?>'
	var table = $('#table-body');
	var loading = $('#loading');
		loading.hide(1);
	$('#btn-apply').click(function(){
		...
		var postData	= {'jenis_input':jenis_input, 'nama_barang':nama_barang, 'nama_cust':nama_cust, 'warna':warna, 'tanggal_input':tanggal_input}
