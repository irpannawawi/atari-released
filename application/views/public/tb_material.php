	...
	function search(event){
		...
		if (event.keyCode === 13 && $('#searchBox').val() != '') {
			...
			var keyword = $('#searchBox').val();
				...
				loading.fadeIn(500);
					$.post("<?=site_url('Lap_material/material_search')?>", {'keyword':keyword}, function(data){
						table.html(data);
						table.show(100)
						loading.fadeOut(500)
					});
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">History Produksi</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
				...
				<tr>
					<td>
						<select id="nama_supplier" class="form-control">
								<option value="" id="defaultCustomerValue">Nama Supplier</option>
							<?php foreach ($list_nama_supplier as $key) { ?>
...
<section class="content">
	...
	<div class="col-12" id="table-wraper">
		<div id="header-page" hidden>
			...
			<small>Jl. Terusan Suryani No. 243, Babakan, Babakan Ciparay, Bandung Kota - 40222</small>
			<hr />
			<h4 align="center">Laporan Material</h4>
		</div>
	...
	<p align="center">
		<a href="<?=site_url('lap_produksi/index/show_all')?>">Show All</a>
	</p>
...
<script type="text/javascript">
	...
	//filter menu action
	var url = '<?=site_url('Lap_material/material_filtered')?>'
	var url_search = '<?=site_url('Lap_material/material_search')?>'
	var table = $('#table-body');
	var loading = $('#loading');
		loading.hide(1);
	$('#btn-apply').click(function(){
		...
		var postData	= { 'nama_barang':nama_barang, 'nama_supplier':nama_supplier,}
	...
	$('#btn-print').click(function(){
		...
		$('thead').html("<tr><th>No</th><th>Nama Barang</th><th>Nama Supplier</th><th>Keterangan</th><th>Qty</th></tr>");
		$('body').html(header)
