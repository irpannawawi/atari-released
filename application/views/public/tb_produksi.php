	...
	function search(event){
		...
		if (event.keyCode === 13) {
			...
			var keyword = $('#searchBox').val();
				...
				loading.fadeIn(500);
					$.post("<?=site_url('lap_barang/barang_search')?>", {'keyword':keyword}, function(data){
						table.html(data);
						table.show(100)
						loading.fadeOut(500)
					});
				...
				<div class="col-9">
					<div class="form-inline">
						<div class="form-group">
							<input type="search" id="searchBox"  class="form-control" onkeyup="search(event)">
						</div>
						<div class="form-group">
...
<script type="text/javascript">
	...
	//filter menu action
	var url = '<?=site_url('lap_barang/barang_filtered')?>'
	var url_search = '<?=site_url('lap_barang/barang_search')?>'
	var table = $('#table-body');
	var loading = $('#loading');
		loading.hide(1);
